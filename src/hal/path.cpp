...
Result<std::filesystem::path> resolve_under_workspace(std::string_view workspace_dir,
    ...
    if (workspace_dir.empty())
        return fail<std::filesystem::path>(ErrorCode::InvalidConfig, "workspace_dir is empty.");
    ...
    if (ec)
        canonical_ws = std::filesystem::absolute(ws, ec);
    if (ec)
        return fail<std::filesystem::path>(ErrorCode::FilesystemError,
                                           "Cannot resolve workspace: " + ec.message());

    auto canonical = std::filesystem::weakly_canonical(joined, ec);
    if (ec)
        canonical = std::filesystem::absolute(joined, ec);
    if (ec)
        return fail<std::filesystem::path>(ErrorCode::FilesystemError,
...
bool is_workspace_root(std::string_view workspace_dir, const std::filesystem::path &path) {
    std::error_code ec;
    auto canonical_ws =
        std::filesystem::weakly_canonical(std::filesystem::path(std::string(workspace_dir)), ec);
    if (ec)
        return false;
