...
Result<ProcessResult> run_shell_windows(std::string_view working_directory,
    ...
    if (!CreatePipe(&read_pipe, &write_pipe, &sa, 0))
        return fail<ProcessResult>(ErrorCode::FilesystemError, "CreatePipe failed.");
    ...
    BOOL created = CreateProcessW(nullptr, wcmd.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW,
    ...
    const ULONGLONG limit_ms =
        timeout_seconds > 0 ? static_cast<ULONGLONG>(timeout_seconds) * 1000ull : 0;
        ...
        if (PeekNamedPipe(read_pipe, nullptr, 0, nullptr, &avail, nullptr) && avail > 0) {
            ...
            if (ReadFile(read_pipe, buf, chunk, &got, nullptr) && got > 0) {
                if (output.size() < max_output_bytes) {
                    ...
                    size_t take =
                        static_cast<size_t>(got) < room ? static_cast<size_t>(got) : room;
                    output.append(buf, take);
                    if (take < static_cast<size_t>(got))
                        output += "\n[output truncated]";
                }
            }
        }
...
Result<ProcessResult> run_shell_posix(std::string_view working_directory,
                                      std::string_view command, int timeout_seconds,
                                      size_t max_output_bytes) {
    int pipefd[2];
    if (pipe(pipefd) != 0)
        return fail<ProcessResult>(ErrorCode::FilesystemError, "pipe() failed.");
    ...
    if (pid == 0) {
        ...
        if (timeout_seconds > 0) {
            ...
            execlp("timeout", "timeout", t.c_str(), "sh", "-c", cmd.c_str(),
    ...
    if (waitpid(pid, &status, 0) < 0)
        return fail<ProcessResult>(ErrorCode::FilesystemError, "waitpid() failed.");
...
Result<ProcessResult> run_shell_command(std::string_view working_directory,
                                        ...
                                        size_t max_output_bytes) {
    if (command.empty())
        return fail<ProcessResult>(ErrorCode::InvalidConfig, "Missing command.");
#if defined(AGENT_HAL_WINDOWS)
