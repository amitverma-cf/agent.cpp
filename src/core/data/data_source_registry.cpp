...
Result<std::string> data_source_tool_callback(std::string_view arguments, void *user_data) {
    ...
    if (!source || !source->query) return fail<std::string>(ErrorCode::InvalidConfig, "DataSource has no query function.");
...
Result<void> register_data_source(Session &session, DataSource ds) {
    if (ds.name.empty()) return fail(ErrorCode::InvalidConfig, "DataSource requires a non-empty name.");
...
Result<std::string> query_data_source(Session &session, std::string_view name, std::string_view query) {
    ...
    if (it == session.data_sources.end()) return fail<std::string>(ErrorCode::KeyNotFound, "Data source not found: " + std::string(name));
    const DataSource &ds = it->second;
    if (!ds.query) return fail<std::string>(ErrorCode::InvalidConfig, "Data source has no query function: " + std::string(name));
