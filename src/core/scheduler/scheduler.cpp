    ...
    for (const auto &e : subagents_)
        if (e.id == id)
            return fail(ErrorCode::InvalidConfig,
                        "AgentScheduler: subagent id already registered: " + id);
...
Result<void> AgentScheduler::add_cron(std::string id, CronTask::TaskFn fn, void *user_data,
                                      std::chrono::milliseconds delay,
                                      std::chrono::milliseconds interval) {
    for (const auto &e : cron_jobs_)
        if (e.task.id == id)
            return fail(ErrorCode::InvalidConfig,
                        "AgentScheduler: cron id already registered: " + id);
...
Result<void> AgentScheduler::cancel_cron(std::string_view id) {
    ...
    return fail(ErrorCode::KeyNotFound,
                "AgentScheduler: cron job not found: " + std::string(id));
}
...
Result<void> AgentScheduler::pump(Session &session) {
    ...
    for (auto &entry : cron_jobs_) {
        if (entry.done || now < entry.task.next_fire)
            continue;

        utils::log(session, LogLevel::Debug,
                   "AgentScheduler: cron '" + entry.task.id + "' fired.");
        ...
        if (entry.task.callback) {
            ...
            if (!res.ok)
                utils::log(session, LogLevel::Warning,
                           "AgentScheduler: cron '" + entry.task.id +
        ...
        if (entry.task.interval.count() > 0) {
            ...
            if (entry.task.next_fire < now)
                entry.task.next_fire = now + entry.task.interval;
        } else {
            entry.done = true;
        }
    }
    ...
    auto finish = [&](SubAgent &entry, Result<bool> res) {
        if (!res.ok) {
            ...
            utils::log(session, LogLevel::Warning,
                       "AgentScheduler: subagent '" + entry.id +
                           "' failed: " + res.error.message);
        } else if (!res.value) {
            entry.done = true;
            ...
            utils::log(session, LogLevel::Info,
                       "AgentScheduler: subagent '" + entry.id + "' completed.");
        }
    };

    const bool provider_can_run_parallel =
        session.config.provider == AiProvider::OpenAICompatible ||
        session.config.provider == AiProvider::LlamaCpp;
    ...
    if (provider_can_run_parallel) {
        ...
        for (size_t i = 0; i < batch.size(); ++i) {
            utils::log(session, LogLevel::Debug,
                       "AgentScheduler: stepped subagent '" + batch[i]->id + "' (parallel).");
        ...
        for (auto &entry : subagents_) {
            ...
            utils::log(session, LogLevel::Debug,
                       "AgentScheduler: stepping subagent '" + entry.id + "'.");
