...
Result<void> init_onnx(Session &session) {
    if (session.config.model.empty()) {
        return fail(ErrorCode::InvalidConfig, "OnnxRuntime requires a model path in Config::model.");
    }

    const OrtApi *api = OrtGetApiBase()->GetApi(ORT_API_VERSION);
    if (!api) {
        return fail(ErrorCode::ProviderInitFailed, "Failed to get ORT API pointer.");
    }
...
#else
    ORT_CHECK_INIT(
        api, api->CreateSession(state->env, session.config.model.c_str(), state->session_opts, &state->ort_session));
#endif
...
Result<ChatResponse> execute_turn_onnx(Session &session, const ChatRequest &request) {
    ...
    std::vector<OrtChar *> raw_input_names(input_count);
            ...
            for (const auto &tv : mv.tensors) {
                ...
                ORT_CHECK(api,
                          api->CreateTensorWithDataAsOrtValue(
                              cpu_mem, const_cast<void *>(static_cast<const void *>(tv.data.data())),
                              tv.data.size_bytes(), tv.shape.data(), tv.shape.size(), to_ort_type(tv.dtype), &val));
                input_values[i] = val;
                break;
            }
            if (input_values[i])
    ...
    std::vector<OrtChar *> raw_output_names(output_count);
    for (size_t i = 0; i < output_count; ++i) {
        ORT_CHECK(api, api->SessionGetOutputName(state->ort_session, i, allocator, &raw_output_names[i]));
    }

    std::vector<const OrtChar *> in_ptrs(input_count), out_ptrs(output_count);
    for (size_t i = 0; i < input_count; ++i)
        in_ptrs[i] = raw_input_names[i];
    for (size_t i = 0; i < output_count; ++i)
        out_ptrs[i] = raw_output_names[i];

    std::vector<OrtValue *> output_values(output_count, nullptr);
    ORT_CHECK(api, api->Run(state->ort_session, nullptr, in_ptrs.data(), input_values.data(), input_count,
                            out_ptrs.data(), output_count, output_values.data()));
    ...
    for (size_t i = 0; i < output_count; ++i) {
        ...
        for (size_t d = 0; d < ndim; ++d)
            elem_count *= static_cast<size_t>(shape_span[d]);
