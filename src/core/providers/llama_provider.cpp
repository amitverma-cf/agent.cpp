  ...
  public:
    ...
    Result<llama_seq_id> acquire_seq() {
        ...
        if (free_seq_ids_.empty())
            return fail<llama_seq_id>(ErrorCode::ProviderInitFailed,
    ...
    void seed_free_list(int n_seq_max) {
        ...
        for (int i = n_seq_max - 1; i >= 0; --i) free_seq_ids_.push_back(i);
    }

    Result<llama_token> prefill(llama_seq_id seq, const std::vector<llama_token> &tokens, llama_sampler *sampler) {
        if (tokens.empty()) return fail<llama_token>(ErrorCode::InvalidConfig, "LlamaEngine: empty prompt.");
        ...
        if (rc != 0) return fail<llama_token>(ErrorCode::DecodeFailed, "LlamaEngine: prefill decode failed.");
    ...
    Result<llama_token> next_token(llama_seq_id seq, llama_token last_token, llama_sampler *sampler) {
        ...
        if (batching_) {
            ...
            results_.erase(seq);
            if (!res.ok) return fail<llama_token>(ErrorCode::DecodeFailed, "LlamaEngine: batched decode failed.");
            return ok(res.token);
        }
        ...
        for (size_t i = 0; i < round.size(); ++i) {
            ...
            RoundResult rr;
            rr.ok = (rc == 0);
            if (rr.ok) rr.token = llama_sampler_sample(round[i].sampler, ctx, static_cast<int32_t>(i));
            results_[s] = rr;
        }
...
Result<void> init_llama_cpp(Session &session) {
    if (session.config.model.empty()) return fail(ErrorCode::InvalidConfig, "llama.cpp requires a model path.");

    auto params = llama_model_default_params();
    llama_model *model = llama_model_load_from_file(session.config.model.c_str(), params);
    if (!model) return fail(ErrorCode::ModelLoadFailed, "Failed to load llama.cpp model from: " + session.config.model);

    const int n_seq_max = std::max(1, session.config.max_parallel_subagents);

    auto ctx_params = llama_context_default_params();
    ctx_params.n_ctx = static_cast<uint32_t>(session.config.context_window) * static_cast<uint32_t>(n_seq_max);
...
Result<void> clear_kv_cache_llama_cpp(Session &session) {
    ...
    if (!engine || !engine->ctx) return fail(ErrorCode::ProviderInitFailed, "Llama model not initialized.");
    llama_memory_clear(llama_get_memory(engine->ctx), true);
    return ok();
}

static bool find_balanced_json_object(std::string_view text, size_t from, size_t &obj_start, size_t &obj_end) {
...
static std::string format_prompt_templated(const llama_model *model, std::span<const MessageView> messages) {
    // Intentional exception boundary (the one exception to this codebase's no-exceptions
    // policy): llama_chat_apply_template is vendored llama.cpp code whose exception-safety
    // this project doesn't control, so any exception it throws is caught here and treated as
    // a template-formatting failure, falling back to the untemplated prompt in the caller.
    try {
        ...
        size_t guess = 0;
        for (const auto &msg : messages) guess += msg.role.size() + msg.content.size() + 8;
        guess = guess * 2 + 256;
        ...
        if (static_cast<size_t>(written) > buf.size()) {
            ...
            written = llama_chat_apply_template(tmpl, chat.data(), chat.size(), true, buf.data(), static_cast<int32_t>(buf.size()));
            if (written < 0) return {};
        }
...
Result<InferResponse> infer_llama_cpp(Session &session, const InferRequest &request) {
    ...
    if (prompt.empty()) prompt = format_prompt_manual(request.messages);

    const llama_vocab *vocab = llama_model_get_vocab(engine->model);

    std::vector<llama_token> tokens(prompt.length() + 4);
    int n_tokens =
        llama_tokenize(vocab, prompt.data(), static_cast<int>(prompt.length()), tokens.data(), static_cast<int>(tokens.size()), true, true);
    if (n_tokens < 0) {
        tokens.resize(static_cast<size_t>(-n_tokens));
        n_tokens = llama_tokenize(vocab, prompt.data(), static_cast<int>(prompt.length()), tokens.data(), static_cast<int>(tokens.size()),
                                  true, true);
    }
    tokens.resize(static_cast<size_t>(n_tokens));

    auto seq_res = engine->acquire_seq();
    if (!seq_res.ok) return fail<InferResponse>(seq_res.error.code, seq_res.error.message);
    ...
    std::string response;
    const int max_gen = request.max_tokens > 0 ? request.max_tokens : session.config.max_tokens > 0 ? session.config.max_tokens : 512;
    ...
    for (int i = 0; i < max_gen; ++i) {
        ...
        if (n >= 0) {
            ...
            if (request.stream && request.on_token) request.on_token(piece, request.token_user_data);
        }
    ...
    if (!session.config.tools.empty()) {
        ...
        while (find_balanced_json_object(response_sv, search_from, obj_start, obj_end)) {
            ...
            if (ambient::json_parser.iterate(padded).get(doc)) continue;

            simdjson::ondemand::object obj;
            if (doc.get_object().get(obj)) continue;

            std::string_view tool_name;
            if (obj["name"].get_string().get(tool_name) || tool_name.empty()) continue;

            auto raw_args_res = obj["arguments"];
            std::string args_str;
            std::string_view raw_json;
            if (!raw_args_res.get_string().get(raw_json)) args_str = std::string(raw_json);
            else if (!raw_args_res.raw_json().get(raw_json)) args_str = std::string(raw_json);
            ...
            while (!visible.empty() && std::isspace(static_cast<unsigned char>(visible.back()))) visible.remove_suffix(1);
            msg.content = visible;
            break;
        }
    }

    return ok(InferResponse{.message = msg,
...
Result<int> count_tokens_llama_cpp(Session &session, std::string_view text) {
    auto engine = std::static_pointer_cast<LlamaEngine>(session.provider_state);
    if (!engine || !engine->model) return fail<int>(ErrorCode::ProviderInitFailed, "Llama model not initialized.");

    const llama_vocab *vocab = llama_model_get_vocab(engine->model);
    std::vector<llama_token> tokens(text.size() + 4);
    int n = llama_tokenize(vocab, text.data(), static_cast<int>(text.size()), tokens.data(), static_cast<int>(tokens.size()), false, false);
    if (n < 0) return ok(static_cast<int>((text.size() + 3) / 4));
    return ok(n);
}
